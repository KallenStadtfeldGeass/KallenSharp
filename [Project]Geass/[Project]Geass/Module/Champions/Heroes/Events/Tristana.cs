        ...
        private void AutoEvents(EventArgs args)
        {
            ...
            if (StaticObjects.ProjectMenu.Item($"{basename}.UseR").GetValue<bool>())
                foreach (var enemy in Functions.Objects.Heroes.GetEnemies(R.Range).OrderBy(hp => hp.Health))
                {
                    ...
                    if ((StaticObjects.Player.GetAutoAttackDamage(enemy) > enemy.Health) &&
                        !StaticObjects.Player.IsWindingUp)
                        continue;
        ...
        private void Clear()
        {
            ...
            if (StaticObjects.ProjectMenu.Item($"{basename}.UseE").GetValue<bool>())
                if (_manaManager.CheckClearE())
                {
                    if (StaticObjects.ProjectMenu.Item($"{basename}.UseE.OnTurrets").GetValue<bool>())
                    {
                        var turrets =
                            ObjectManager.Get<Obj_AI_Turret>()
                                .OrderBy(dis => dis.ServerPosition.Distance(StaticObjects.Player.ServerPosition));
                    ...
                    {
                        var bigmonsters =
                            MinionManager.GetMinions(E.Range, MinionTypes.All, MinionTeam.Neutral)
                                .Where(
                                    name =>
                                        !name.Name.ToLower().Contains("mini") &&
                                        !name.SkinName.ToLower().Contains("mini"))
                    ...
                    {
                        var validMinons = MinionManager.GetMinions(StaticObjects.Player.Position, E.Range - 50,
                            MinionTypes.All, MinionTeam.NotAlly);
                        if (validMinons.Count >=
                            StaticObjects.ProjectMenu.Item($"{basename}.MinionsInRange").GetValue<Slider>().Value)
                        {
        ...
        private void Combo()
        {
            ...
            if (StaticObjects.ProjectMenu.Item($"{basename}.UseE").GetValue<bool>())
                if (_manaManager.CheckComboE())
                    ...
                    {
                        if (
                            !StaticObjects.ProjectMenu.Item($"{basename}.UseE.On.{target.ChampionName}")
                                .GetValue<bool>())
                            continue;
        ...
        private void OnDraw(EventArgs args)
        {
            ...
            if (E.Level > 0)
                if (
                    ...
                    Render.Circle.DrawCircle(StaticObjects.Player.Position, E.Range,
            ...
            {
                var stackedTarget =
                    Functions.Objects.Heroes.GetEnemies(1000).FirstOrDefault(x => x.HasBuff("TristanaECharge"));
