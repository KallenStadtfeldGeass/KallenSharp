...
namespace _Project_Geass.Module.Champions.Heroes.Events
{
    internal class Ezreal : Base
    {
            ...
            if (!_tearFull)
                if (!StaticObjects.Player.IsRecalling())
                {
                    ...
                    if (StaticObjects.ProjectMenu.Item($"{basename}.UseQ.TearStack").GetValue<bool>() &&
                        (_manaManager.ManaPercent >= StaticObjects.ProjectMenu.Item($"{basename}.UseQ.TearStack.MinMana").GetValue<Slider>().Value))
                    ...
                    if (StaticObjects.ProjectMenu.Item($"{basename}.UseQ.Minon.LastHit").GetValue<bool>())
                    {
                                ...
                                .OrderBy(hp => hp.Health))
                        {
                            if (!StaticObjects.Player.IsWindingUp &&
                                StaticObjects.Player.GetAutoAttackDamage(target) < target.Health + 25 && StaticObjects.Player.Distance(target) < StaticObjects.Player.AttackRange) continue;
        ...
        // if (DelayCheck + _lastTick > Functions.AssemblyTime.CurrentTime()) if
        // (Items.HasItem(LeagueSharp.Common.Data.ItemData.Tear_of_the_Goddess.Id)) { _tearFull =
        // _lastMana >= StaticObjects.Player.MaxMana; _lastMana = StaticObjects.Player.MaxMana; }
