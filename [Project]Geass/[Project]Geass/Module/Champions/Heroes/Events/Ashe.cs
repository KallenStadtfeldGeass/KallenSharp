            ...
            if (StaticObjects.ProjectMenu.Item($"{basename}.UseR").GetValue<bool>())
                if (_manaManager.CheckComboR())
                {
                    ...
                    if (focusTarget != null)
                        if (
                            StaticObjects.ProjectMenu.Item($"{basename}.UseR.On.{focusTarget.ChampionName}")
                                .GetValue<bool>())
                            if (
                                StaticObjects.ProjectMenu.Item($"{basename}.UseR.On.{focusTarget.ChampionName}.HpMin")
                                    .GetValue<Slider>()
                                    .Value > focusTarget.HealthPercent)
                                if (
                                    StaticObjects.ProjectMenu.Item(
                    ...
                    if (!focusTargetValid)
                    {
                        ...
                        {
                            if (
                                !StaticObjects.ProjectMenu.Item($"{basename}.UseR.On.{target.ChampionName}")
                                    .GetValue<bool>()) continue;
                            if (
                                StaticObjects.ProjectMenu.Item($"{basename}.UseR.On.{target.ChampionName}.HpMin")
                                    .GetValue<Slider>()
                                    .Value > target.HealthPercent) continue;
                            if (
                                StaticObjects.ProjectMenu.Item($"{basename}.UseR.On.{target.ChampionName}.HpMax")
                                    .GetValue<Slider>()
                                    .Value < target.HealthPercent) continue;
