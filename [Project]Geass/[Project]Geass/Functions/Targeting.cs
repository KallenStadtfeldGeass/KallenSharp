...
namespace _Project_Geass.Functions
{
    internal class Targeting
    {
        ...
        /// <returns></returns>
        public static IOrderedEnumerable<PredictionWithChampion> GetTargetPredictions2(Spell spell,
            HitChance minHitChance, List<string> validChampNames)
        {
            var tempList = new List<PredictionWithChampion>();

            foreach (
                var champion in
                Heroes.GetEnemies(spell.Range).Where(name => validChampNames.Contains(name.ChampionName)))
            {
                if (champion.HasBuffOfType(BuffType.Invulnerability) || champion.HasBuffOfType(BuffType.SpellImmunity) ||
                    champion.HasBuffOfType(BuffType.SpellShield)) continue;
        ...
        /// <param name="validChampNames">The valid champ names.</param>
        /// <returns></returns>
        public static IOrderedEnumerable<PredictionOutput> GetTargetPredictions(Spell spell, HitChance minHitChance,
            List<string> validChampNames)
        {
